...
real(dp)                                        ::      a               ! for FLAG
real(dp)                                        ::      timefunction, sumdf, sumdi, numf, numi
!real(dp),allocatable, dimension(:,:)            ::      RESULTS
integer                                         ::      sizeofRESULTS, num
real(dp)                                        ::      ItoR, mean, SD, sumval
real(dp)                                        ::      timeint, maxtime
real(dp),allocatable, dimension(:)              ::      sumdipolfinal, sumdipolinital, cmiu
integer ,allocatable, dimension(:)              ::      numtotfinal, numtotinital
...
allocate(sumdipolfinal(INT(maxtime/timeint)+1))
allocate(sumdipolinital(INT(maxtime/timeint)+1))
allocate(numtotfinal(INT(maxtime/timeint)+1))
allocate(numtotinital(INT(maxtime/timeint)+1))
allocate(cmiu(INT(maxtime/timeint)+1))
!RESULTS=0.0_dp
sumdipolfinal  = 0.0_dp
sumdipolinital = 0.0_dp
numtotfinal    = 0
numtotinital   = 0
...
!    write(*,*) file2
  ...
  call RotationalCorrelationFunction(Ads,Water,waterID,Xwi,Ywi,Zwi,AdsID,Xai,Yai,Zai,Xwf,Ywf,Zwf,Xaf,Yaf,Zaf,&
       timefunction,sumdf,sumdi,numf,numi)
  
  sumdipolfinal(kkk)  = sumdipolfinal(kkk)  + sumdf
  sumdipolinital(kkk) = sumdipolinital(kkk) + sumdi
  numtotfinal(kkk)    = numtotfinal(kkk)    + numf
  numtotinital(kkk)   = numtotinital(kkk)   + numi
write(*,*) "Final dipol is " , sumdipolfinal(kkk), " initial dipol is " ,sumdipolinital(kkk),&
            " numtotfin=",numtotfinal(kkk), "numtotin=",numtotinital(kkk)
...
!write(*,*) "dipolfinal ", sumdipolfinal

!do iii=1,INT(maxtime/timeint)+1
!    write(*,*) ,(sumdipolfinal(iii)/numtotfinal(iii))/(sumdipolinital(iii)/numtotinital(iii))
!end do
...
write   ( 5000, 5010)
5010 format ( 3x, "t(ps)", 11x, "Cmio(t)", 16x,"Inital-Water", 16x, "Final-Water")
write   (5000,5020)
5020 format (2x, "=============================================",&
                "================================================")
do iii=1,INT(maxtime/timeint)+1
   cmiu(iii)=(sumdipolfinal(iii)/numtotfinal(iii))/(sumdipolinital(iii)/numtotinital(iii))
   write   (5000, 5030) iii*timeint, cmiu(iii), numtotinital(iii), numtotfinal(iii)
   5030 format (2x, F6.2, 5x , ES20.13, 12x, I5, 20x, I5)
end do
...
!write(*,*) a        !FLAG
deallocate(sumdipolfinal, sumdipolinital,numtotfinal, numtotinital,cmiu, stat = alloc_err)

end program fun


subroutine RotationalCorrelationFunction(Ads,Water,waterID,Xwi,Ywi,Zwi,AdsID,Xai,Yai,Zai,Xwf,Ywf,Zwf,Xaf,Yaf,Zaf,&
           timefunction,sumdf,sumdi,numf,numi)
implicit none
...
real(dp),                             intent(out)                 :: timefunction, sumdf, sumdi, numf, numi
...
!write(*,*) " Sumdipolinitial is " , sumdi, " and Sumdipolfinal is ", sumdf
